...
const Switch = ({ checked, onChange }) => {
  const toggleId = `sparkle-toggle-${Math.random().toString(36).substr(2, 9)}`;
  
  return (
    <StyledWrapper>
      <div className={`toggle-cont ${checked ? 'checked' : ''}`}>
        <input 
...
const StyledWrapper = styled.div`
  ...
  .toggle-cont .toggle-label:active {
  ...
  .toggle-cont.checked .toggle-label .cont-icon {
    ...
    border-bottom: 0;
    box-shadow: inset 0 -0.15rem 0.15rem var(--primary),
      inset 0 0 0.5rem 0.75rem var(--primary);
